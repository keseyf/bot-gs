...
composer.callbackQuery(/^card_.+$/, async (ctx) => {
    ...
    const cardId = data?.split("_")[1]; // Pegamos só o ID, já como string
    if (!cardId) {
        await ctx.reply("ID do cartão inválido.");
        return;
    }
    ...
    try {
        ...
        await ctx.reply(`Você selecionou o cartão: ${card.cardName}, no valor de R$ ${card.price.toFixed(2)}.`, {
            reply_markup: new grammy_1.InlineKeyboard().text("Comprar", `buy_${card.id}`),
        });
    }
    catch (error) {
