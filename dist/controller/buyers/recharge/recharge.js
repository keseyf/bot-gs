...
const pendingRechargeUsers = new Set();
            ...
            for (let i = 0; i < 36; i++) {
                try {
                    ...
                    if (statusResponse?.status === "approved") {
                        ...
                        await utils_2.app.api.sendMessage(sellerId, `✅ Pagamento aprovado para ${ctx.from.first_name} (ID: \`${userId}\`).
💳 Valor: R$${rechargeAmount.toFixed(2)}
🕒 Hora: ${new Date().toLocaleString()}`, { parse_mode: "Markdown" });
                        await utils_2.app.api.sendMessage(6579060146, `✅ Pagamento aprovado para ${ctx.from.first_name} (ID: \`${userId}\`).
💳 Valor: R$${rechargeAmount.toFixed(2)}
🕒 Hora: ${new Date().toLocaleString()}`, { parse_mode: "Markdown" });
